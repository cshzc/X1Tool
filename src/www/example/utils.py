...
class Utils(object):
    ...
    def get_default_apps(self):
        ...
        front_end_apps = {'programmer' : [
                            {'app':'hash', 'template':'programmer/hash.tpl'},
                            {'app':'base64', 'template':'programmer/base64.tpl'},
                            {'app':'timestamp', 'template':'programmer/timestamp.tpl'},
                            {'app':'crypto', 'template':'programmer/crypto.tpl'},
                            {'app':'qrcode', 'template':'programmer/qrcode.tpl'}]}
        ...
        for category_info in registered_category:
            for category_id in category_info:
                ...
                try:
                    if front_end_apps[category_info[category_id]['name']] is not None:
                        applications[category_info[category_id]['name']] = dict(applications[category_info[category_id]['name']], **front_end_apps[category_info[category_id]['name']])
                except:
                    pass
    ...
    '''
        return [
                    [
                        {'name':'programmer', 'icon':'fa-desktop'},
                        {'name':'finance', 'icon':'fa-bank'},
                        {'name':'life', 'icon':'fa-coffee'}
                    ],
                    {
                        'programmer' : [
                            {'app':'hash', 'template':'programmer/hash.tpl'}, 
                        ...
                        'finance' : [
                            {'app':'income_tax', 'template':'finance/income_tax.tpl'}, 
                        ],
                        'life' : [
                            {'app':'express', 'template':'life/express.tpl'}, 
                        ]
                    }
                ]
    '''
