...
function Process(){
  SetTab();
  window.IsCollapsible = $id("collapsible_view").checked;
  var json = $id("raw_string").value;
  var html = "";
  try{
    if(json == ""){ json = "\"\"";return;}
    var obj = eval("["+json+"]");
    html = ProcessObject(obj[0], 0, false, false, false);
    $id("lint_result").innerHTML = "<PRE class='CodeContainer'>"+html+"</PRE>";
  }catch(e){
    alert("JSON数据格式不正确:\n"+e.message);
    $id("lint_result").innerHTML = "";
  }
}
...
function QuoteKeysClicked(){
  ...
  window.QuoteKeys = $id("quote_keys").checked;
...
function CollapseAllClicked(){
  ...
  TraverseChildren($id("lint_result"), function(element){
...
function CollapseLevel(level){
  ...
  TraverseChildren($id("lint_result"), function(element, depth){
...
function EnsureIsPopulated(){
  ...
  if(!$id("lint_result").innerHTML && !!$id("raw_string").value) Process();
...
function SelectAllClicked(){
  ...
  if(!!range.selectNode)
    ...
    range.selectNode($id("lint_result"));

  else if(range.moveToElementText)

    range.moveToElementText($id("lint_result"));

  

  if(!!range.select)

    range.select($id("lint_result"));
